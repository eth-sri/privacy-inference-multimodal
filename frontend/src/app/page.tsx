...
import { FreeTextAttributes } from "@/components/FreeTextAttributes";
...
export default function Home() {
  ...
  // State to track the input for the new attribute name
  const [newAttributeName, setNewAttributeName] = useState("");

  // Modified function to use newAttributeName when adding a new attribute
  const addOtherAttribute = () => {
    if (newAttributeName.trim() === "") {
      alert("The attribute name cannot be empty!");
      return;
    }

    if (formData.others.hasOwnProperty(newAttributeName)) {
      alert("An attribute with this name already exists!");
      return;
    }

    setOtherAttributeNames([...otherAttributeNames, newAttributeName]);

    setFormData({
      ...formData,
      others: {
        ...formData.others,
        [newAttributeName]: defaultAttributeDict,
      },
    });

    // Reset the input for new attribute name
    setNewAttributeName("");
  };
  ...
  const fetchUser = async () => {
    await fetch(`${backendURL}/reddit/?username=${datapoint?.author}`)
  ...
  const fetchDatapoint = async () => {
    await fetch(`${backendURL}/get_datapoint/`)
      .then((response) => response.json())
      .then((data) => {
        setDatapoint(convertResponseToState(data.datapoint));
      })
      .catch((error) => {
        console.log(error);
      });
      ...
      <main className="flex flex-col min-h-screen p-5">
        <div className="grid grid-cols-2 gap-4 flex-grow">
          ...
          <div className="flex flex-col">
            ...
            <div>
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex mb-1">
                  {/* Input field for new attribute name */}
                  <Input
                    value={newAttributeName}
                    onChange={(e) => setNewAttributeName(e.target.value)}
                    placeholder="Enter attribute name such as interests, medical, pet, hobbies.."
                    className="max-w-[500px] mr-1 my-1"
                  />
                  {/* Add Button to create a new attribute */}
                  <Button
            ...
            <div className="mb-1">
              <Button className="mr-1 bg-red-700" onClick={handleNext}>
                Next
              </Button>
              <Button onClick={handleSkip} className="mb-1 bg-blue-700">
                Skip
              </Button>
              <Button
