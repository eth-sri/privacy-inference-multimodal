    ...
    <Card className="w-full grid grid-cols-5 mb-2">
      ...
      <CardContent className="col-span-4">
        <div className="grid grid-cols-4 w-full items-center gap-2">
          <div className="flex flex-col">
            {type === "text" && (
              <Input
                ...
                onChange={(event) => handleInputChange("estimate", event)}
                placeholder={name}
              />
            )}
            {type === "select" && select && (
              ...
              >
                ...
                <SelectContent position="popper">
                  {select.map((select, id) => (
                    <SelectItem key={id} value={select.label}>
                      {select.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {type === "radio" && select && (
              <RadioGroup
                ...
                defaultValue=""
                value={value.estimate}
                onValueChange={(value) => handleRadioChange("estimate", value)}
              >
                {select.map((select, id) => (
            ...
            {type === "textarea" && (
              <Textarea placeholder="Other Attributes"></Textarea>
            )}
          </div>
          <div className="flex flex-col items-center">
            <Select
              value={value.hardness === 0 ? "" : value.hardness?.toString()}
              onValueChange={(value) =>
                handleSelectChange("hardness", Number(value))
              }
            >
          ...
          <div className="flex flex-col items-center">
            <Select
              value={value.certainty === 0 ? "" : value.certainty?.toString()}
              onValueChange={(value) =>
                handleSelectChange("certainty", Number(value))
              }
            >
          ...
          <div className="flex flex-col items-center">
            <Select
              value={
                value.information_level === 0
                  ? ""
                  : value.information_level?.toString()
              }
              onValueChange={(value) =>
                handleSelectChange("information_level", Number(value))
              }
            >
